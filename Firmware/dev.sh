...
# those is still usable on its own — CI and the git hook call them directly rather than
# through here, so there is one definition of each job.
#
set -euo pipefail
...
case "$cmd" in
    ...
    *)
        ...
        echo "Try: play | test | format | check | host | build | flash | all | suite | display | touchpad | touchdot | user_button" >&2
        exit 2
        ;;
esac
