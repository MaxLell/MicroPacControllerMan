...
i2c_bsp_status_e touchpad_probe(void)
{
    ...
    return i2c_bsp_read_memory(TOUCHPAD_DEVICE_ADDRESS, TOUCHPAD_REGISTER_FIRMWARE_MAJOR,
                               I2C_BSP_MEMORY_ADDRESS_WIDTH_8_BIT, firmware_version, sizeof(firmware_version));
}
...
i2c_bsp_status_e touchpad_read(touchpad_reading_t* out_reading)
{
    ...
    status = i2c_bsp_read_memory(TOUCHPAD_DEVICE_ADDRESS, TOUCHPAD_REGISTER_TOUCH_STATE,
                                 I2C_BSP_MEMORY_ADDRESS_WIDTH_8_BIT, touch_block, sizeof(touch_block));
    if (status != I2C_BSP_STATUS_OK)
    {
        return status;
    }

    out_reading->is_touched = (touch_block[TOUCHPAD_TOUCH_STATE_INDEX] & TOUCHPAD_TOUCH_STATE_TOUCH_PRESENT) != 0U;

    if (out_reading->is_touched)
    {
        out_reading->x = (uint16_t)(((uint16_t)touch_block[TOUCHPAD_TOUCH_X_INDEX] << TOUCHPAD_LSB_NIBBLE_BITS)
                                    | (touch_block[TOUCHPAD_TOUCH_LSB_INDEX] >> TOUCHPAD_LSB_X_SHIFT));

        out_reading->y = (uint16_t)(((uint16_t)touch_block[TOUCHPAD_TOUCH_Y_INDEX] << TOUCHPAD_LSB_NIBBLE_BITS)
