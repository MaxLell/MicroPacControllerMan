...
static void prv_select(bool in_is_selected)
{
    ...
    dio_bsp_set_pin(DIO_BSP_PIN_DISPLAY_CS,
                    in_is_selected ? DIO_BSP_PIN_STATE_LOW : DIO_BSP_PIN_STATE_HIGH);
}
...
static void prv_command_with_data(uint8_t in_command, const uint8_t* const in_data,
                                  size_t in_length)
{
    prv_select(true);
    prv_write_command(in_command);
...
static void prv_open_window(uint16_t in_x, uint16_t in_y, uint16_t in_width, uint16_t in_height)
{
    ...
    const uint8_t columns[] = {(uint8_t)(in_x >> 8), (uint8_t)in_x, (uint8_t)(x_end >> 8),
                               (uint8_t)x_end};
    const uint8_t rows[] = {(uint8_t)(in_y >> 8), (uint8_t)in_y, (uint8_t)(y_end >> 8),
                            (uint8_t)y_end};
...
bool st7789_is_present(void)
{
    ...
    return (id[0] == ST7789_ID_EXPECTED_0) && (id[1] == ST7789_ID_EXPECTED_1)
                           ...
                           uint16_t in_colour)
{
    ...
    while (remaining > 0U)
    {
        const uint32_t pixels = (remaining < ST7789_FILL_CHUNK_PIXELS) ? remaining
                                                                       : ST7789_FILL_CHUNK_PIXELS;
