...
| `joystick/` | The GFX01M2 shield's five-key joystick: five instances of `switch`, all active low, with a latched press edge per key so one press is one step. Polled from the same 1 ms tick as `user_button`. |
...
**concrete instances** (`user_button`, `joystick`) — five keys needed no change to the
primitive, which is the point of the split. What the 32-sample window costs in input
