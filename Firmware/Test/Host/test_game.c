...
void test_a_second_run_starts_from_scratch(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0U, game_get_score(&g_game));
    TEST_ASSERT_EQUAL_UINT8(GAME_STARTING_LIVES, game_get_lives(&g_game));
    TEST_ASSERT_TRUE(msg_cell_bitmap_get(prv_state().has_pellet, ABOVE_START_X, ABOVE_START_Y));
}
...
void test_pacman_moves_one_cell_per_period(void)
{
    ...
    TEST_ASSERT_EQUAL_INT16(ABOVE_START_X, prv_state().pacman.column);
    TEST_ASSERT_EQUAL_INT16(ABOVE_START_Y, prv_state().pacman.row);
    TEST_ASSERT_EQUAL(DIRECTION_NORTH, prv_state().pacman.direction);
}
...
void test_leftover_time_is_carried_into_the_next_move(void)
{
    ...
    TEST_ASSERT_EQUAL_INT16(ABOVE_START_Y - 1, prv_state().pacman.row);
}
...
void test_eating_a_pellet_scores_and_clears_the_cell(void)
{
    ...
    TEST_ASSERT_FALSE(msg_cell_bitmap_get(prv_state().has_pellet, ABOVE_START_X, ABOVE_START_Y));
}
...
void test_progress_is_zero_at_the_moment_of_a_step(void)
{
    game_start(&g_game);
    game_set_direction(&g_game, DIRECTION_NORTH);
    game_tick(&g_game, GAME_PACMAN_MOVE_PERIOD_MS);

    TEST_ASSERT_EQUAL_UINT8(0U, prv_state().pacman.progress);
}

void test_progress_grows_between_two_steps(void)
{
    uint8_t quarter;
    uint8_t half;
    ...
    game_start(&g_game);
    game_set_direction(&g_game, DIRECTION_NORTH);
    game_tick(&g_game, GAME_PACMAN_MOVE_PERIOD_MS); /* land on a cell, timer at zero */

    game_tick(&g_game, GAME_PACMAN_MOVE_PERIOD_MS / 4U);
    quarter = prv_state().pacman.progress;

    game_tick(&g_game, GAME_PACMAN_MOVE_PERIOD_MS / 4U);
    half = prv_state().pacman.progress;

    /* Roughly a quarter and a half of 256 — the exact values follow from integer division,
     * so the test pins the shape rather than the arithmetic. */
    TEST_ASSERT_GREATER_THAN_UINT8(0U, quarter);
    TEST_ASSERT_GREATER_THAN_UINT8(quarter, half);
    TEST_ASSERT_LESS_THAN_UINT8(255U, half);
}

void test_progress_stays_at_zero_when_the_step_is_blocked(void)
{
    game_start(&g_game);

    /* Row 3 of the level-1 maze is "#.........#", so walking west from column 2 reaches
     * column 1 and then stops against the wall — still *facing* west, which is what §10.1
     * says a blocked entity does. Its movement timer keeps running, so interpolating along
     * that facing would slide the sprite into the wall and snap it back. */
    pacman_reset(&g_game.pacman, prv_make_cell(2, 3));
    ...
    game_tick(&g_game, GAME_PACMAN_MOVE_PERIOD_MS);
    game_tick(&g_game, GAME_PACMAN_MOVE_PERIOD_MS / 2U);

    TEST_ASSERT_EQUAL(DIRECTION_WEST, prv_state().pacman.direction);
    TEST_ASSERT_EQUAL_UINT8(0U, prv_state().pacman.progress);
}

void test_progress_never_reports_arrival(void)
{
    game_start(&g_game);
    game_set_direction(&g_game, DIRECTION_NORTH);

    /* One tick short of the period: the sprite must still be short of the next cell,
     * because saying "arrived" is the model's job, not the view's. */
    game_tick(&g_game, GAME_PACMAN_MOVE_PERIOD_MS - 1U);

    TEST_ASSERT_LESS_THAN_UINT8(255U, prv_state().pacman.progress);
}
