...
./format.sh --check         # report offenders and exit 1 — this is the CI gate
...
build (and CI) only ever *checks*, and the rewriting happens on save in the editor or in
the pre-commit hook. Wire `format-check` into CI; leave `format` a manual target.
