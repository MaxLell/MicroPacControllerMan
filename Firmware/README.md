...
The coding standard (NFR-102) is `.clang-format`, vendored from
[c-code-style](https://github.com/MaxLell/c-code-style). `format.sh` applies it. Needs
clang-format 14 or newer (`sudo apt-get install -y clang-format`; verified on 18.1.3):

```
./format.sh                 # format everything in place
./format.sh --check         # report offenders and exit 1 — the gate
./format.sh --diff          # show exactly what --check objects to
./format.sh App/game        # just one path
./format.sh --staged        # only what is staged, and re-stage it (the hook uses this)
```
...
./dev.sh remove-hook
./dev.sh pre-commit         # the same thing by hand
git commit --no-verify      # skip it once
```

The hook runs the cheap half of `check` — formatting and the host unit tests, about five
seconds incrementally — and skips itself entirely when a commit stages no C sources, so a
docs-only commit stays instant. It deliberately does **not** run the OTTs: those need the
board plugged in and a human watching it, which is not a thing to owe on every commit. The
cross-build is left out for time, and catches little the host build does not. Before
opening a PR, run the fuller **`./dev.sh check`** — formatting, unit tests, and both
builds, writing nothing.

One limitation worth knowing: the hook tests the working tree, not the index. With
unstaged changes around, what runs is not exactly what is being committed. Testing the
index properly needs a throwaway checkout, which is more machinery than this earns.
...
**Neither is part of the default build, on purpose.** A build that rewrites its own
sources changes files the compiler has already stamped — the next incremental build is
then non-deterministic, it fights whatever editor has the file open, and it silently
rewrites work you have not read yet. The usual arrangement is the other way round: the
build only ever *checks*, and the rewriting happens on save in the editor or in the
pre-commit hook below. There is no CI in this repo yet; when there is, `./dev.sh check` is
the job.

`ThirdParty/` is never formatted — the CubeMX export is regenerated by the GUI and
EmbeddedCli is vendored upstream, so reformatting either would turn every future update
into a merge conflict. A block that has to keep a hand-made shape is guarded in the source
with `/* clang-format off */` … `/* clang-format on */`; there is exactly one today, the
maze table in `App/playfield/playfield.c`, where one row per line *is* the maze.
