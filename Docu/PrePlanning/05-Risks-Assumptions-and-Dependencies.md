...
| R-009 | The X-NUCLEO-GFX01M2 pin map is derived from UM2750's assignments for the *neighbouring* NUCLEO-G4 family, because that document lists no STM32U5 board at all. It is an inference, not a measurement. | High — a wrong pin map blocks all display and input bring-up, and the symptom is hard to distinguish from a software fault. | Medium | Confirm every signal on the board with a logic analyzer before writing any display driver. Map and reasoning in [M2 Board Bring-Up §1](../Design/M2-Board-Bring-Up.md). |
| R-010 | Display CS lands on **PA9**, which is USART1_TX — the ST-LINK virtual COM port carrying the console and the whole OTT mechanism. CS is hardwired on the shield and cannot move, and it is active *high*, so leaving PA9 as a transmit line would select and deselect the display mid-transfer. | High — blocks all display work, and would cost the project its on-target test channel if resolved the wrong way. | Confirmed — not a risk of occurring but a problem to solve. | Establish whether the NUCLEO-U545RE-Q can re-route the VCP off USART1 by solder bridge; that needs the board's user manual. Failing that, either give up the on-board VCP for an external USB-serial adapter on free pins, or rework the shield's CS line. Detail in [M2 Board Bring-Up §1.2](../Design/M2-Board-Bring-Up.md). |
| R-011 | Display SCK lands on **PA5**, which is also LD2, the green user LED — a property of the Nucleo-64 boards themselves. Once SPI1 drives the pin, LD2 becomes an activity indicator rather than an independent output, and the `blinky` OTT that drives and reads it back stops being valid. | Low — cosmetic for the game, but it invalidates an existing test. | Confirmed. | Retire or rework the `blinky` OTT when the display is wired up. Noted in [M2 Board Bring-Up §1.2](../Design/M2-Board-Bring-Up.md). |
