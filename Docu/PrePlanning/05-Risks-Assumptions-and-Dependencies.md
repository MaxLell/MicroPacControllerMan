...
| R-009 | The shield's ST-morpho *positions* are known from UM2750, but which STM32U545RE pin sits at each position is not: UM2750 lists no STM32U5 board at all, so its GPIO columns describe other boards. The board-side table is in UM3062, which is not yet in hand. | High — a wrong pin map blocks all display and input bring-up, and the symptom is hard to distinguish from a software fault. | Medium | Obtain UM3062 for the CN7/CN10 assignment, then confirm on the board — the joystick lines can be measured in firmware, the display lines need a logic analyzer. Positions and reasoning in [M2 Board Bring-Up §1](../Design/M2-Board-Bring-Up.md). |
| R-010 | Display CS would land on **PA9**, which is USART1_TX — the ST-LINK virtual COM port carrying the console and the whole OTT mechanism. CS is hardwired on the shield and cannot move, and it is active *high*, so leaving PA9 as a transmit line would select and deselect the display mid-transfer. | High — blocks all display work, and would cost the project its on-target test channel if resolved the wrong way. | Likely — depends on R-009: it follows from the G4 reference mapping, not from a U545RE-Q table. | Establish whether the NUCLEO-U545RE-Q can re-route the VCP off USART1 by solder bridge; that needs the board's user manual. Failing that, either give up the on-board VCP for an external USB-serial adapter on free pins, or rework the shield's CS line. Detail in [M2 Board Bring-Up §1.2](../Design/M2-Board-Bring-Up.md). |
| R-011 | Display SCK would land on **PA5**, which is also LD2, the green user LED — a property of the Nucleo-64 boards themselves. Once SPI1 drives the pin, LD2 becomes an activity indicator rather than an independent output, and the `blinky` OTT that drives and reads it back stops being valid. | Low — cosmetic for the game, but it would invalidate an existing test. | Likely — depends on R-009, same as R-010. | Retire or rework the `blinky` OTT when the display is wired up. Noted in [M2 Board Bring-Up §1.2](../Design/M2-Board-Bring-Up.md). |
...
| D-008 | UM2750 (shield) and **UM3062** (STM32U3/U5 Nucleo-64 boards, MB1841) | UM2750 is in hand. UM3062 carries the CN7/CN10 pin assignment this board's pin map depends on (R-009) and still has to be obtained; st.com downloads have failed from the development host. |
