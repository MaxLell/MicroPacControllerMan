...
| R-009 | The X-NUCLEO-GFX01M2 pin map is established on paper by cross-referencing UM2750 and UM3062 by connector position, but has not been confirmed on the board. | Medium — a wrong pin map blocks display bring-up and the symptom is hard to distinguish from a software fault. | Low — two authoritative ST documents agree by position. | Confirm the display lines with a logic analyzer before trusting a misbehaving driver; the joystick lines can be verified in firmware without an instrument. Map in [M2 Board Bring-Up §1](../Design/M2-Board-Bring-Up.md). |
| R-011 | Display SCK is PA5, which also drives LD2 — a property of the Nucleo-64 board. The LED is a capacitive load on a clock line intended to run at tens of MHz. | Low — resolved by a documented solder bridge, at the cost of the LED. | Confirmed by UM3062 table 18. | Set solder bridge **SB10 OFF**, which UM3062 table 19 provides for exactly this case. LD2 and the `blinky` OTT are given up in exchange. Detail in [M2 Board Bring-Up §1.2](../Design/M2-Board-Bring-Up.md). |
