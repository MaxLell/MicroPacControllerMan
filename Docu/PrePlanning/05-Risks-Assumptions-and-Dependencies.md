...
| R-009 | The X-NUCLEO-GFX01M2 pin map is established by cross-referencing UM2750 and UM3062 by connector position. The five joystick pins are confirmed on hardware; the seven display pins are not yet observed. | Medium — a wrong display pin blocks bring-up and looks like a software fault. | Low — the method is validated where it has been checked, and two authoritative ST documents agree by position. | Joystick: **closed** by the `joystick` OTT, which also confirmed each key matches its compass name. Display: confirm with a logic analyzer, or treat the first bring-up as the measurement, remembering that a wrong pin shows up as no reaction at all. Detail in [M2 Board Bring-Up §1](../Design/M2-Board-Bring-Up.md). |
