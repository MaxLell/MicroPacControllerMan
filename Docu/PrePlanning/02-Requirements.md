...
| FR-037 | AI Play Strength | On a full run of the **normal maze** (FR-040), with every rule of the game in force, the AI shall reach a final score of at least 4,600 points, and more than a uniform-random policy on the same maze. *(One run, because the normal maze is one maze and the game has nothing random in it, so a trained network plays the identical episode every time. 4,600 is about ten times the measured mean of a uniform-random policy — 433.5 points over 20 episodes of this maze — and clearing level 1 alone is worth 2,600. Both figures tunable; amended by [DEC-045](11-Decisions-and-As-Built.md), which moved training and this measurement off the generated mazes of seeds 1000..1019. See [M6 Pacman AI §13](../Design/M6-Pacman-AI.md))* |
