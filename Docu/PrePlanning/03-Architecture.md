...
One folder is not a layer. **`Training/`** holds the host-only machine-learning harness of Milestone 6 — the `ctypes` shim over the game, the evolution loop and the weight exporter ([DEC-040](11-Decisions-and-As-Built.md)). It is neither firmware nor verification: it produces an artefact (`App/pacman_ai/ai_weights.c`) that the firmware then contains. It builds only in the host configuration, and the dependency is one-way — Python may depend on the game, the firmware build may never depend on Python (CON-105).
