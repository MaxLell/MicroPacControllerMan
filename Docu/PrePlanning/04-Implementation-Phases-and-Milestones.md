...
| 4 | System Integration (Target) | Milestones 2 and 3 exit met. | All remaining integration tests pass on the physical target ([VT-INT-011..013](06-Verification-and-Validation.md), VT-INT-015, VT-INT-016, VT-INT-022). VT-INT-009 was withdrawn with the RTOS ([DEC-027](11-Decisions-and-As-Built.md)), so this milestone no longer carries an RTOS bring-up: what is left is the measurement M3 did not make automatic — input latency (NFR-003) and the frame rate under a real run (NFR-002). |
