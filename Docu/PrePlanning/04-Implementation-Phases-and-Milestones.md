...
| 1 | Toolchain Bring-Up | Milestone 0 exit met. | The OTT CLI framework (FR-106/FR-107) runs on the target over the ST-LINK V3 serial console, drivable by the Python harness. The retained-RAM/reset mechanism ([doc 09](09-OTT-Mechanism-and-Reset-Flow.md)) — originally planned for M2 — was brought forward and built/validated here, so `ott <name>` already schedules via `.noinit`, resets, and reports PASS/FAIL on the next boot. *(Toolchain bring-up was originally proven with a register-level blinky LED on LD2/PA5 (VT-INT-005); that LED was **retired in M2** when the design moved to CubeMX/HAL and PA5 was reassigned to SPI1_SCK for the display — the on-board LED is no longer part of the project. See [11 Decisions & As-Built](11-Decisions-and-As-Built.md).)* |
